module.exports = app => {
    ...
    router.get('/news/init', async (req, res) => {
        ...
        await Article.insertMany(newsList);  //录入数据
        res.send(newsList)
    })
    /** 导入新闻数据 end  **/
    ...
    router.get('/news/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            { $match: { parent: parent._id } }, //过滤数据，查询相同的parent._id下的数据
            {
                $lookup: {//关联查询
                    from: 'articles',
                    localField: '_id',
    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            { $match: { parent: parent._id } }, //过滤数据,条件查询 类似与MySQL的where
            {
                $lookup: {//关联查询
                    from: 'heroes',  //集合名称
                    localField: '_id',
    ...
    //英雄详情
    router.get('/heroInfo/:id',async (req,res)=>{
