...
//5.fs.readdir 读取目录  把目录下的文件或者文件夹都获取到 例：[ 'fs.js', 't1.txt' ]
...
//6.fs.rename 重命名   1）改名；2）剪切文件

/*
//1)改名
fs.rename('html/fs.js','html/new.js',function(err){
    if(err){
        console.log(err);
        return false;
    }
...
//2）剪切文件
/*fs.rename('style.css','css/newStyle.css',function(err){
    if(err){
        console.log(err);
        return false;
    }
